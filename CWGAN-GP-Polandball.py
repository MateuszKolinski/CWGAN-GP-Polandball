...
easyPatterns = ["Abbasid", "Armenia", "Austria", "Austrian Empire",  "Bangladesh", "Belgium", "Bolivia (no CoA)", "Botswana", "Bulgaria", "Catalonia", "Colombia", "Congo - Republic", "Crimea", "Czech Republic", "Denmark", "England", "Estonia", "Faroe Islands", "Finland", "France", "German Empire", "Germany", "Greenland", "Guinea", "Hungary", "Iceland", "Ireland", "Italy", "Jamaica", "Kuwait", "L+-beck OR Bohemia OR actual Poland", "Laos", "Latvia", "Libya (1977-2011)", "Lithuania", "Luxembourg", "Madagascar", "Mali", "Mauritius", "Netherlands", "Netherlands (orange)", "Niger", "Nigeria", "Norway", "Palau", "Palestine", "Papal States", "Peru", "Poland OR Indonesia OR Monaco", "Qatar", "Romania OR Chad", "Russia", "Scania", "Scotland", "Sealand", "Serbia (civil)", "Shetland OR Greece (1822-1969 and 1975-78)", "South Vietnam", "Spain (no CoA)", "Sudan", "Sweden", "Switzerland", "Tanzania", "Thailand", "Ukraine", "uland", "United Arab Emirates", "Yemen", "Yugoslavia (no star)"]
mediumPatterns = ["7ball", "8ball", "Algeria", "Argentina", "Belarus", "Brazil", "Burkina Faso", "Cameroon", "Chile", "Congo - Dr", "Cuba", "Dominican Republic", "East Timor", "East Turkistan", "French Guiana", "Ghana", "Greece", "Guinea-Bissau", "Guyana", "Honduras", "India", "Japanese Army", "Jordan", "Kurdistan", "Lebanon", "Liberia", "Libya", "Liechtenstein", "Malaysia", "Maldives", "Malta", "Micronesia", "Morocco", "Mozambique", "Myanmar", "Namibia", "Nauru", "North Korea", "North Macedonia", "Pakistan", "Panama", "Philippines", "Puerto Rico", "Senegal", "Seychelles", "Somalia", "South Africa", "South Sudan", "Suriname", "Texas", "Togo", "Tonga", "Tunisia", "Turkey", "Uruguay", "Vietnam", "Yugoslavia (star)"]
hardPatterns = ["Afghanistan", "Albania", "Angola", "Australia", "Azerbaijan", "Bavaria", "Bolivia (CoA)", "Bosnia and Herzegovina", "Byzantine Empire", "Cambodia", "Canada", "Cape Verde", "China", "Croatia", "Cyprus", "East Germany", "Ecuador", "Egypt", "El Salvador", "Eritrea", "Ethiopia", "European Union", "Georgia", "Guatemala", "Haiti", "Hong Kong", "Iran", "Iraq", "Italian Empire", "Japan", "Kingdom of France", "Kiribati", "Kosovo", "Kyrgyzstan", "Mauritania (1959-2017)", "Mexico", "Moldova", "Mongolia", "Montenegro", "New Zealand", "Nicaragua", "Northern Ireland (Ulster)", "Papua New Guinea", "Paraguay", "Portugal", "Prussia", "Quebec", "Republic of China", "Romani", "Saudi Arabia", "Serbia", "Slovakia", "Slovenia", "South Korea", "Spain (CoA)", "Sri Lanka", "Syria (green 3 stars)", "Syria (red 2 stars)", "Turkmenistan", "United Kingdom", "United Nations", "United States", "USSR", "Uzbekistan", "Vatican City", "Venezuela (no COA)", "Wales", "Zimbabwe"]
...
def show_tensor_images(image_tensor, num_images=25, size=(3, 64, 64), upcolor=True):
...
# Removes corners of an image in a circular fashion #
def crop_circle(file_path=None, image=None):
    if file_path is not None:
        image = Image.open(file_path)

    # Create white circle on black background #
    # "L" is a PIL Image mode representing 8bit grayscale pixels #
...
def training(paths, parameters, misc, device):
    ...
    dataset = PolandballDataset(paths["path_csv_database"], paths["path_augmented_RGB_images"], transform=transform, remove_alpha=True)
    ...
    generator_input_dim, critic_image_channels = get_input_dimensions(parameters["z_dim_generator"], parameters["image_channels"], n_classes)
    generator = Generator(kernels=parameters["generator_kernels"], strides=parameters["generator_strides"], input_dim=generator_input_dim).to(device)
    critic = Critic(kernels=parameters["critic_kernels"], strides=parameters["critic_strides"], image_channels=critic_image_channels).to(device) 
    ...
    for epoch in range(parameters["n_epochs"]):
        ...
        for real, labels in dataloader:
            ...
            one_hot_labels = torch.nn.functional.one_hot(labels.to(device), n_classes) #2
            image_one_hot_labels = one_hot_labels[:, :, None, None] #2
            image_one_hot_labels = image_one_hot_labels.repeat(1, 1, parameters["image_dimensions"], parameters["image_dimensions"]) #2
            ...
            for _ in range(parameters["critic_repeats"]):
                ...
                critic_fake_pred = critic(fake_image_and_labels.detach())
                critic_real_pred = critic(real_image_and_labels)

                epsilon = torch.rand(len(real), 1, 1, 1, device=device, requires_grad=True)
                gradient = get_gradient(critic, real_image_and_labels, fake_image_and_labels.detach(), epsilon)
            ...
            noise_and_labels_2 = combine_vectors(noise_2, one_hot_labels)
            fake_2 = generator(noise_and_labels_2)
            fake_image_and_labels_2 = combine_vectors(fake_2, image_one_hot_labels)
            critic_fake_pred = critic(fake_image_and_labels_2)
    ...
    save_info_to_file(paths["path_training_info"], misc, parameters, class_names, time_spent)

    # Save image plot of losses #
    create_matplot(losses_steps, losses_generator, losses_critic, epoch_steps, paths["path_training_info"])

    create_results(parameters, misc, n_classes, device, class_names, paths, generator_input_dim, dataset)
...
def collect_training_images_for_gif(n_classes, z_dim_generator, generator, training_progress_images, training_progress_full_images, device):
    ...
    for desired_class in range(n_classes):
        ...
        image = crop_circle(image=desired_image)
...
def sample_generator(desired_class, n_classes, z_dim_generator, generator, device):
    ...
    return generator(noise_and_labels)
...
def save_info_to_file(path_training_info, misc, parameters, class_names, time_spent):
...
def create_database_csv(csv_file_path, database_paths, file_name):
    class_names = {}
    class_index = -1
        ...
        for database_path in database_paths:
            ...
            for full_file_name in full_file_names:
                ...
                if full_file_name_FF.file_extension in common_image_extensions:
                    class_name = full_file_name.split('_', 1)[0]
                    if class_name not in class_names:
                        class_index = class_index + 1
                        class_names[class_name] = class_index

                    writer.writerow([full_file_name, class_index])

    # Rewrite file but this time sorted NATURALLY (natsort) #
    data = csv.reader(open(os.path.join(csv_file_path, file_name)))
    natsort_key1 = natsort_keygen(alg=ns.IGNORECASE)
    sortedlist = sorted(data, key=natsort_key1)
    with open(os.path.join(csv_file_path, file_name), 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["Filename", "Label"])
        for sortedLine in sortedlist:
            writer.writerow([sortedLine[0], sortedLine[1]])
...
# Transforming images and saving them in a separate directory, prior to training #
def preTransform(path_csv, path_original_images, path_resized_original_class, path_augmented_RGB_class, image_dimensions):
    read = pd.read_csv(path_csv)
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Resize((image_dimensions, image_dimensions))
        ])
    dataset = PolandballDataset(path_csv, path_original_images, transform=transform)
    class_names = dataset.get_class_names()
    required_n_class = {}
    class_overflow = {}
    for class_name in class_names:
        class_overflow[class_name] = 0
        if class_name in easyPatterns:
            required_n_class[class_name] = 100

        if class_name in mediumPatterns:
            required_n_class[class_name] = 200

        if class_name in hardPatterns:
            required_n_class[class_name] = 300

    i = 0
    dataloader = DataLoader(dataset, shuffle=False)
    for real, _ in dataloader:
        class_name = dataset.get_file_name(i).split("_", 1)[0]
        id = int(dataset.get_file_name(i).split("_", 1)[1].split(".", 1)[0])
        ratio = required_n_class[class_name] / dataset.get_class_num(class_name)
        ratio = math.ceil(ratio)
        if ratio % 2 == 0:
            ratio = ratio + 1
        image = tensor_to_Image(real)
        image.save(os.path.join(path_resized_original_class, class_name + "_" + str(id + class_overflow[class_name]) + ".png"))
        image.convert("RGB")
        image.save(os.path.join(path_augmented_RGB_class, class_name + "_" + str(id + class_overflow[class_name]) + ".png"))
        class_overflow[class_name] = class_overflow[class_name] + 1
        basic_rotation_angle = 10
        for x in range(ratio-1):
            real_product = transforms.functional.rotate(real, (-1)**x * basic_rotation_angle)
            if x % 2 == 1:
                basic_rotation_angle = basic_rotation_angle - 2
            image = tensor_to_Image(real_product)
            image.convert('RGB')
            file_name = dataset.get_file_name(i).split("_", 1)[0] + "_" + str(id + class_overflow[class_name]) + ".png"
            image.save(os.path.join(path_augmented_RGB_class, file_name))
            class_overflow[class_name] = class_overflow[class_name] + 1
...
# Directory and file shenaningans, creating csv and pretransforming #
def prepare_database(path_original_images, path_resized_original_images, path_augmented_RGB_images, image_dimensions):
    # Loop over ball folders #
    original_ball_folders = [f.path for f in os.scandir(path_original_images) if f.is_dir()]
    for original_ball_folder in original_ball_folders:
        # Helper class #
        original_ball_folder_FF = FFile(original_ball_folder, "folder")

        # Rename files in folder to class + _ + index #
        rename_images_in_folder(original_ball_folder_FF.full)

        # Create csv file containing ball files #
        ...
        # Create subdirectories for all classes for resized originals and transformed images #
        path_csv = os.path.join(original_ball_folder_FF.full, str(original_ball_folder_FF.file_name + "Database.csv"))
        path_resized_original_class = os.path.join(path_resized_original_images, original_ball_folder_FF.file_name)
        if (os.path.isdir(path_resized_original_class)) is False:
            os.mkdir(path_resized_original_class)
        path_augmented_RGB_class = os.path.join(path_augmented_RGB_images, original_ball_folder_FF.file_name)
        if (os.path.isdir(path_augmented_RGB_class)) is False:
            os.mkdir(path_augmented_RGB_class)

        # Pretransform original images, resizing them, removing alpha channel and augmenting #
        preTransform(path_csv, path_original_images, path_resized_original_class, path_augmented_RGB_class, image_dimensions)
...
class Generator(nn.Module):
    def __init__(self, kernels, strides, input_dim=64, image_channels=3, hidden_dim=32):
...
class Critic(nn.Module):
    def __init__(self, kernels, strides, image_channels=3, hidden_dim=64):
...
def setup_paths(learning_set_name, dt_string):
    ...
    paths["path_augmented_RGB_images"] = os.path.join(paths["path_images"], "PreparedImages")
    ...
    paths["path_csv_database"] = os.path.join(paths["path_augmented_RGB_images"], "PolandballCsv.csv")

    if os.path.isdir(paths["path_resized_original_images"]) is False:
        os.mkdir(paths["path_resized_original_images"])

    if os.path.isdir(paths["path_augmented_RGB_images"]) is False:
        os.mkdir(paths["path_augmented_RGB_images"])
...
def setup_parameters():
    ...
    # GAN Architecture #
    parameters["generator_kernels"] = [2, 4, 4, 4, 6]
    parameters["generator_strides"] = [1, 2, 2, 2, 2]
    parameters["critic_kernels"] = [4, 4, 4, 6]
    parameters["critic_strides"] = [2, 2, 2, 2]

    # Generated image characteristics #
    parameters["image_dimensions"] = 64
    parameters["image_channels"] = 3
...
def setup_misc():
    ...
    misc["learning_set_name"] = "Polandball"
...
def main():
    ...
    prepare_database(paths["path_original_images"], paths["path_resized_original_images"], paths["path_augmented_RGB_images"], parameters["image_dimensions"])

    # Create main database file by scanning all ball folders #
    ball_folders = [f.path for f in os.scandir(paths["path_augmented_RGB_images"]) if f.is_dir()]
    create_database_csv(paths["path_augmented_RGB_images"], ball_folders, "PolandballCsv.csv")
